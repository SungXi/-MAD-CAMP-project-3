...
#                             utils.print_fused_image(batch_image, self.train_out_dir, str(iter)+'_pre_train.png', 2)
                        
#                         if np.mod(iter+1, self.pre_train_iter) == 0:
#                             self.saver.save(sess, self.save_dir+ '/pre_train', global_step=iter)
                        
            #Training iterations
            if os.path.isfile(self.save_dir+ '/model-{}.meta'.format(39999)):
                self.saver.restore(sess, self.save_dir+ '/model-'+str(39999))
                print('Finished loading train model')
                
            for iter in range(40000, self.iter):                
                    ...
                    if np.mod(iter+1, 500) == 0:
                        batch_image = sess.run([self.fake_cartoon], 
                                               feed_dict={self.photo_input: photo_batch, 
                                                          self.is_train: True})
                        batch_image = np.squeeze(batch_image)
                        utils.print_fused_image(batch_image, self.train_out_dir, str(iter)+'.png', 2)
    ...
    def test(self):
        ...
        self.test_list = utils.get_filename_list('test_real_world')
        ...
        for idx in range(100):
            ...
            utils.print_fused_image(images, self.test_out_dir, str(idx)+'.png', 2)
            
            photo_batch = np.squeeze(photo_batch)
            utils.print_fused_image(photo_batch, self.test_out_dir, str(idx)+'.jpg', 2)
            
#         for idx, image in enumerate(self.test_list):
#             img = cv2.imread(image)
#             img = cv2.resize(img, dsize=(256, 256), interpolation = cv2.INTER_LINEAR)
#             img = img / 127.5 - 1
#             img = np.array([img])
#             img = self.sess.run([self.fake_cartoon], feed_dict={self.photo_input: img, self.is_train: True})
#             img = np.array(img[0])
#             img = (img[0] + 1) * 127.5
#             cv2.imwrite('test_real_world\%d%d%d.jpg' %(idx+1, idx+1, idx+1), img)
